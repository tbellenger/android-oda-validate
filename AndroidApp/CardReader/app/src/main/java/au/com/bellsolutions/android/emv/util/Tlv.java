package au.com.bellsolutions.android.emv.util;
...
public class Tlv implements Parcelable {
	...
	public static Tlv parseTlv(byte[] buff, int offset) throws TlvException {
		...
		if ((buff[offset + lengthTag] & 0x80) == 0x80) { // high-bit set - multi length
			...
			byte[] len = reverseArray(buff, offset + lengthTag + 1, count);
	 ...
	 */
	public static byte[] reverseArray(byte[] array, int start, int len) {
		byte[] out = new byte[len];
		
		for (int i = len, j = 0; i > 0; i--, j++) {
			out[i - 1] = array[start + j];
		}
	...
	public void writeToParcel(Parcel dest, int flags) {
		dest.writeString(HexString.hexify(mBuffer));
	}
	
	public static final Parcelable.Creator<Tlv> CREATOR = new Parcelable.Creator<Tlv>() {
        public Tlv createFromParcel(Parcel source) {
            Tlv t = new Tlv();
			try {
				t = Tlv.parseTlv(HexString.parseHexString(source.readString()));
			} catch (TlvException e) {
